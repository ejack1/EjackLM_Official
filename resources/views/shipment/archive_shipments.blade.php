...
@section('title')
    <title>Archived Shipments</title>
@endsection

@section('content')
<h1 style="text-align: center; font-size:2.4rem; padding-top:20px" >Archived Shipments</h1>
...
<div class="search hidden sm:block" style="margin: 10px;">
    <form action="{{url('shipment_search')}}" method="post">
        @csrf
        {{-- @method('PUT') --}}

        <input type="text" name="search_query" class="search__input input placeholder-theme-13" placeholder="Reference ID...">
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                {{-- <td>{{$t->shipment_status}}</td> --}}
                @if($t->shipment_status)
                <td>{{$t->shipment_status->shipment_status_title}}</td>
                @endif

                {{-- <td>
                    @foreach($t->shipment_status as $v)
                        <p> {{$v->shipment_status_value}} </p>
                    @endforeach
                </td> --}}

                {{-- <td>
                
                    @foreach($t->shipment_status as $value)
                        <p>{{$value->shipment_status_title}}</p>
                    @endforeach
                ...
                <td>
                    ...
                    <a href="{{ url('shipment_De_archive',$t->id) }}">  
